"""Level 5 - The refusal direction (REFERENCE SOLUTION). The finale.
...
def refusal_direction(model, tok, harmful: list[str], harmless: list[str],
    ...
    h = torch.stack([_resid_last(model, tok, p, layer) for p in harmful]).mean(0)
    g = torch.stack([_resid_last(model, tok, p, layer) for p in harmless]).mean(0)
    d = h - g
    return d / d.norm()
